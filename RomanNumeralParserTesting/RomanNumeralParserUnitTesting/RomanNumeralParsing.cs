...
namespace RomanNumeralParser
{
    class Program
    {
        ...
        {
            public static List<int> numbersOutput;
            public static Dictionary<Numerals, Char> numeralPairs; 
            public static accumulators Accumulators;
            public static string InputString;
            public static bool IsDone;
            public static Action executionProcess;
        }
        ...
        static void Init()
        {
            attributeset.numbersOutput = new List<int>();
            attributeset.numeralPairs = new Dictionary<Numerals, char>();

            attributeset.numeralPairs = SetNumeralPairs();
            attributeset.InputString = "";
            attributeset.IsDone = false;

            attributeset.executionProcess =
                () =>
                {
                    attributeset.InputString = GetStringToken();

                    attributeset.numbersOutput = DiassembleStringToTokens(attributeset.InputString);

                    attributeset.Accumulators = AssembleOutput(attributeset.numbersOutput);

                    WriteIntTokens(attributeset.numbersOutput, attributeset.Accumulators, attributeset.InputString);

                    attributeset.IsDone = IsDone();
                };
        ...
        static Numerals GetNumerals(Char searchChar)
        {
            ...
            foreach (var pair in attributeset.numeralPairs)
            {
        ...
        {
            return (int)GetNumerals(attributeset.numeralPairs[numeral]);
        }
